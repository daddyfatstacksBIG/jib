...
public class FailoverHttpClientTest {
  ...
  private interface CallFunction {
    ...
    Response call(FailoverHttpClient httpClient, URL url, Request request)
        throws IOException;
  }
  ...
  private final GenericUrl fakeUrl =
      new GenericUrl("https://crepecake/fake/url");
  private final LongAdder totalByteCount = new LongAdder();

  @Before
  public void setUp() throws IOException {
    ByteArrayInputStream inStream =
        new ByteArrayInputStream(new byte[] {'b', 'o', 'd', 'y'});
  ...
  public void testHttpTimeout_doNotSetByDefault() throws IOException {
    try (Response ignored = newHttpClient(false, false)
                                .get(fakeUrl.toURL(), fakeRequest(null))) {
    }

    Mockito.verify(mockHttpRequest, Mockito.never())
  ...
  public void testHttpTimeout() throws IOException {
    ...
    try (Response ignored =
             httpClient.get(fakeUrl.toURL(), fakeRequest(5982))) {
    }
  ...
  public void testGet_nonHttpsServer_insecureConnectionAndFailoverDisabled()
    ...
    try (Response response =
             httpClient.get(new URL("http://plain.http"), fakeRequest(null))) {
      Assert.fail("Should disallow non-HTTP attempt");
    } catch (SSLException ex) {
      Assert.assertEquals(
          "insecure HTTP connection not allowed: http://plain.http",
          ex.getMessage());
    }
  }
  ...
  public void testCall_secureClientOnUnverifiableServer() throws IOException {
    ...
    try (Response response =
             httpClient.get(new URL("https://insecure"), fakeRequest(null))) {
  ...
  public void testGet_insecureClientOnUnverifiableServer() throws IOException {
    ...
    Mockito.when(mockHttpRequest.execute())
        .thenThrow(new SSLPeerUnverifiedException(""));

    try (Response response = insecureHttpClient.get(new URL("https://insecure"),
                                                    fakeRequest(null))) {
  ...
  @Test
  public void testGet_insecureClientOnHttpServerAndNoPortSpecified()
      throws IOException {
    FailoverHttpClient insecureHttpClient = newHttpClient(true, false);
  ...
  @Test
  public void testGet_secureClientOnNonListeningServerAndNoPortSpecified()
      throws IOException {
    FailoverHttpClient httpClient = newHttpClient(false, false);

    Mockito.when(mockHttpRequest.execute())
        .thenThrow(new ConnectException(
            "my exception")); // server not listening on 443
  ...
  @Test
  public void testGet_insecureClientOnNonListeningServerAndPortSpecified()
      throws IOException {
    FailoverHttpClient insecureHttpClient = newHttpClient(true, false);

    Mockito.when(mockHttpRequest.execute())
        .thenThrow(new ConnectException(
            "my exception")); // server is not listening on 5000

    try (Response response = insecureHttpClient.get(
             new URL("https://insecure:5000"), fakeRequest(null))) {
      Assert.fail(
          "Should not fall back to HTTP if port was explicitly given and cannot connect");
    } catch (ConnectException ex) {
      Assert.assertEquals("my exception", ex.getMessage());
  ...
  public void testGet_timeoutFromConnectException() throws IOException {
    ...
    Mockito.when(mockHttpRequest.execute())
        .thenThrow(new ConnectException("Connection timed out"));

    try (Response response = insecureHttpClient.get(new URL("https://insecure"),
                                                    fakeRequest(null))) {
      Assert.fail(
          "Should not fall back to HTTP if timed out even for ConnectionException");
    } catch (ConnectException ex) {
  ...
  public void testGet_doNotSendCredentialsOverHttp() throws IOException {
    ...
    Assert.assertNull(
        httpHeadersCaptor.getAllValues().get(1).getAuthorization());
  }

  @Test
  public void testGet_sendCredentialsOverHttp() throws IOException {
    FailoverHttpClient insecureHttpClient =
        newHttpClient(true, true); // sendCredentialsOverHttp

    try (Response response = insecureHttpClient.get(
             new URL("http://plain.http"), fakeRequest(null))) {
    }

    Assert.assertEquals(1, urlCaptor.getAllValues().size());

    Assert.assertEquals("Basic ZmFrZS11c2VybmFtZTpmYWtlLXNlY3JldA==",
                        httpHeadersCaptor.getValue().getAuthorization());
  }

  @Test
  public void testGet_originalRequestHeaderUntouchedWhenClearingHeader()
      throws IOException {
    FailoverHttpClient insecureHttpClient = newHttpClient(true, false);

    Request request = fakeRequest(null);
    try (Response response =
             insecureHttpClient.get(new URL("http://plain.http"), request)) {
    }
  ...
  public void testShutDown() throws IOException {
    ...
    try (Response response =
             secureHttpClient.get(fakeUrl.toURL(), fakeRequest(null))) {
  ...
  public void testFollowFailoverHistory_insecureHttps() throws IOException {
    ...
    Mockito.when(mockHttpRequest.execute())
        ...
        .thenReturn(mockHttpResponse);
    try (Response response1 =
             httpClient.get(new URL("https://url"), fakeRequest(null));
         Response response2 =
             httpClient.post(new URL("https://url"), fakeRequest(null))) {
    }
  ...
  @Test
  public void testFollowFailoverHistory_httpFailoverByConnectionError()
      throws IOException {
    FailoverHttpClient httpClient = newHttpClient(true, false);
    ...
    verifyWarnings(
        "Failed to connect to https://url over HTTPS. Attempting again with HTTP.");
  }
  ...
  public void testFollowFailoverHistory_httpFailover() throws IOException {
    ...
    try (Response response1 =
             httpClient.get(new URL("https://url:123"), fakeRequest(null));
         Response response2 =
             httpClient.post(new URL("https://url:123"), fakeRequest(null))) {
    }

    Mockito.verify(mockHttpRequest, Mockito.times(3)).execute();
    Mockito.verify(mockInsecureHttpRequest, Mockito.times(1)).execute();
    verifyCapturedUrls("https://url:123", "https://url:123", "http://url:123",
                       "http://url:123");
    verifyWarnings(
  ...
  public void testFollowFailoverHistory_portsDifferent() throws IOException {
    ...
    try (Response response1 =
             httpClient.get(new URL("https://url:1"), fakeRequest(null));
         Response response2 =
             httpClient.post(new URL("https://url:2"), fakeRequest(null))) {
    }

    Mockito.verify(mockHttpRequest, Mockito.times(2)).execute();
    Mockito.verify(mockInsecureHttpRequest, Mockito.times(2)).execute();
    verifyCapturedUrls("https://url:1", "https://url:1", "https://url:2",
                       "https://url:2");
    verifyWarnings(
  ...
  private void setUpMocks(HttpTransport mockHttpTransport,
                          HttpRequestFactory mockHttpRequestFactory,
                          HttpRequest mockHttpRequest) throws IOException {
    Mockito.when(mockHttpTransport.createRequestFactory())
        .thenReturn(mockHttpRequestFactory);
    Mockito
        .when(mockHttpRequestFactory.buildRequest(
            Mockito.any(), urlCaptor.capture(), Mockito.any()))
        .thenReturn(mockHttpRequest);

    Mockito.when(mockHttpRequest.setHeaders(httpHeadersCaptor.capture()))
        .thenReturn(mockHttpRequest);
    Mockito.when(mockHttpRequest.setConnectTimeout(Mockito.anyInt()))
        .thenReturn(mockHttpRequest);
    Mockito.when(mockHttpRequest.setReadTimeout(Mockito.anyInt()))
        .thenReturn(mockHttpRequest);
    Mockito.when(mockHttpRequest.execute()).thenReturn(mockHttpResponse);
  }

  private FailoverHttpClient newHttpClient(boolean insecure,
                                           boolean authOverHttp)
      throws IOException {
    setUpMocks(mockHttpTransport, mockHttpRequestFactory, mockHttpRequest);
    if (insecure) {
      setUpMocks(mockInsecureHttpTransport, mockInsecureHttpRequestFactory,
                 mockInsecureHttpRequest);
    }
    return new FailoverHttpClient(
        insecure, authOverHttp, logger,
        () -> mockHttpTransport, () -> mockInsecureHttpTransport);
  }
  ...
  private Request fakeRequest(Integer httpTimeout) {
    return Request.builder()
        ...
        .setBody(new BlobHttpContent(Blobs.from("crepecake"),
                                     "fake.content.type", totalByteCount::add))
        .setAuthorization(
            Authorization.fromBasicCredentials("fake-username", "fake-secret"))
        .setHttpTimeout(httpTimeout)
        .build();
  }

  private void verifyCall(String httpMethod, CallFunction callFunction)
      throws IOException {
    FailoverHttpClient httpClient = newHttpClient(false, false);
    try (Response ignored = callFunction.call(httpClient, fakeUrl.toURL(),
                                              fakeRequest(null))) {
    }

    Assert.assertEquals("fake.accept,another.fake.accept",
                        httpHeadersCaptor.getValue().getAccept());
    Assert.assertEquals("fake user agent",
                        httpHeadersCaptor.getValue().getUserAgent());
    // Base64 representation of "fake-username:fake-secret"
    Assert.assertEquals("Basic ZmFrZS11c2VybmFtZTpmYWtlLXNlY3JldA==",
                        httpHeadersCaptor.getValue().getAuthorization());

    Mockito.verify(mockHttpRequestFactory)
        .buildRequest(Mockito.eq(httpMethod), Mockito.eq(fakeUrl),
                      blobHttpContentCaptor.capture());
    Assert.assertEquals("fake.content.type",
                        blobHttpContentCaptor.getValue().getType());

    ByteArrayOutputStream byteArrayOutputStream = new ByteArrayOutputStream();
    blobHttpContentCaptor.getValue().writeTo(byteArrayOutputStream);

    Assert.assertEquals("crepecake", byteArrayOutputStream.toString(
  ...
  private void verifyCapturedUrls(String... urls) {
    List<String> captured = urlCaptor.getAllValues()
                                .stream()
                                .map(GenericUrl::toString)

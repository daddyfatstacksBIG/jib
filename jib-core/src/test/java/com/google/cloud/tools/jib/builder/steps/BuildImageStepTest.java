...
public class BuildImageStepTest {
  ...
  @Mock
  private ProgressEventDispatcher.Factory mockProgressEventDispatcherFactory;
  ...
  public void setUp() throws DigestException {
    testDescriptorDigest = DescriptorDigest.fromHash(
        "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");

    Mockito.when(mockBuildContext.getEventHandlers())
    ...
    Mockito.when(mockContainerConfiguration.getCreationTime())
    ...
    nonEmptyLayerHistory = HistoryEntry.builder()
                               .setCreationTimestamp(Instant.EPOCH)
                               .setAuthor("JibBase")
                               .setCreatedBy("jib-test")
                               .build();
    emptyLayerHistory = HistoryEntry.builder()
                            .setCreationTimestamp(Instant.EPOCH)
                            .setAuthor("JibBase")
                            .setCreatedBy("jib-test")
                            .setEmptyLayer(true)
    ...
    baseImage =
        Image.builder(V22ManifestTemplate.class)
            ...
            .setHealthCheck(
                DockerHealthCheck
                    .fromCommand(ImmutableList.of("CMD-SHELL", "echo hi"))
            ...
            .addVolumes(ImmutableSet.of(AbsoluteUnixPath.get("/base/path1"),
            ...
            .build();
    baseImageLayers =
        Arrays.asList(new PreparedLayer.Builder(mockCachedLayer).build(),
                      new PreparedLayer.Builder(mockCachedLayer).build(),
                      new PreparedLayer.Builder(mockCachedLayer).build());
    applicationLayers = Arrays.asList(
        new PreparedLayer.Builder(mockCachedLayer)
  ...
  public void test_basicCase() {
    Image image =
        new BuildImageStep(mockBuildContext, mockProgressEventDispatcherFactory,
                           baseImage, baseImageLayers, applicationLayers)
            .call();
    Assert.assertEquals(
        testDescriptorDigest,
        image.getLayers().asList().get(0).getBlobDescriptor().getDigest());
  }

  @Test
  public void test_propagateBaseImageConfiguration() {
    Mockito.when(mockContainerConfiguration.getEnvironmentMap())
        .thenReturn(ImmutableMap.of("MY_ENV", "MY_ENV_VALUE", "BASE_ENV_2",
                                    "NEW_VALUE"));
    ...
    Mockito.when(mockContainerConfiguration.getVolumes())
        .thenReturn(ImmutableSet.of(AbsoluteUnixPath.get("/new/path1"),
    ...
    Assert.assertEquals(ImmutableMap.of("BASE_ENV", "BASE_ENV_VALUE", "MY_ENV",
                                        "MY_ENV_VALUE", "BASE_ENV_2",
                                        "NEW_VALUE"),
                        image.getEnvironment());
    Assert.assertEquals(ImmutableMap.of("base.label", "base.label.value",
                                        "my.label", "my.label.value",
                                        "base.label.2", "new.value"),
                        image.getLabels());
    Assert.assertNotNull(image.getHealthCheck());
    Assert.assertEquals(ImmutableList.of("CMD-SHELL", "echo hi"),
                        image.getHealthCheck().getCommand());
    Assert.assertTrue(image.getHealthCheck().getInterval().isPresent());
    Assert.assertEquals(Duration.ofSeconds(3),
                        image.getHealthCheck().getInterval().get());
    Assert.assertTrue(image.getHealthCheck().getTimeout().isPresent());
    Assert.assertEquals(Duration.ofSeconds(2),
                        image.getHealthCheck().getTimeout().get());
    Assert.assertTrue(image.getHealthCheck().getStartPeriod().isPresent());
    Assert.assertEquals(Duration.ofSeconds(1),
                        image.getHealthCheck().getStartPeriod().get());
    ...
    Assert.assertEquals(ImmutableSet.of(Port.tcp(1000), Port.udp(2000),
                                        Port.tcp(3000), Port.udp(4000)),
                        image.getExposedPorts());
    Assert.assertEquals(ImmutableSet.of(AbsoluteUnixPath.get("/base/path1"),
                                        AbsoluteUnixPath.get("/base/path2"),
  ...
  public void test_inheritedEntrypoint() {
    ...
    Assert.assertEquals(ImmutableList.of("baseImageEntrypoint"),
                        image.getEntrypoint());
  ...
  public void test_inheritedEntrypointAndProgramArguments() {
    ...
    Assert.assertEquals(ImmutableList.of("baseImageEntrypoint"),
                        image.getEntrypoint());
    Assert.assertEquals(ImmutableList.of("catalina.sh", "run"),
  ...
  public void test_notInheritedProgramArguments() {
    ...
    Assert.assertEquals(ImmutableList.of("myEntrypoint"),
                        image.getEntrypoint());
  ...
  public void test_generateHistoryObjects() {
    ...
    // Base layers (1 non-empty propagated, 2 empty propagated, 2 non-empty
    // generated)
    ...
    Assert.assertEquals(expectedAddedBaseLayerHistory,
                        image.getHistory().get(3));
    Assert.assertEquals(expectedAddedBaseLayerHistory,
                        image.getHistory().get(4));

    // Application layers (4 generated)
    Assert.assertEquals(expectedApplicationLayerHistoryDependencies,
                        image.getHistory().get(5));
    Assert.assertEquals(expectedApplicationLayerHistoryResources,
                        image.getHistory().get(6));
    Assert.assertEquals(expectedApplicationLayerHistoryClasses,
                        image.getHistory().get(7));
    Assert.assertEquals(expectedApplicationLayerHistoryExtrafiles,
                        image.getHistory().get(8));

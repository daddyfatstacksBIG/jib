...
public class DockerConfigCredentialRetrieverTest {
  ...
  public void setUp()
    ...
    dockerConfigFile =
        Paths.get(Resources.getResource("core/json/dockerconfig.json").toURI());
  ...
  public void testRetrieve_nonExistentDockerConfigFile() throws IOException {
    DockerConfigCredentialRetriever dockerConfigCredentialRetriever =
        new DockerConfigCredentialRetriever("some registry",
                                            Paths.get("fake/path"));

    Assert.assertFalse(
        dockerConfigCredentialRetriever.retrieve(mockLogger).isPresent());
  }

  @Test
  public void testRetrieve_hasAuth() throws IOException {
    DockerConfigCredentialRetriever dockerConfigCredentialRetriever =
        new DockerConfigCredentialRetriever("some other registry",
                                            dockerConfigFile);

    Optional<Credential> credentials =
        dockerConfigCredentialRetriever.retrieve(mockLogger);
  ...
  public void testRetrieve_credentialHelperTakesPrecedenceOverAuth() {
    ...
    Mockito.verify(mockLogger)
        .accept(
            LogEvent.info("trying docker-credential-foo for some registry"));
  }
  ...
  public void testRetrieve_credentialHelper_warn()
    ...
    Mockito.when(mockDockerConfig.getCredentialHelperFor("another registry"))
        .thenReturn(mockDockerCredentialHelper);
    Mockito.when(mockDockerCredentialHelper.retrieve())
        .thenThrow(new CredentialHelperNotFoundException(
    ...
    Mockito.verify(mockLogger)
        .accept(LogEvent.warn(
            "The system does not have docker-credential-path CLI"));
  ...
  public void testRetrieve_none() throws IOException {
    DockerConfigCredentialRetriever dockerConfigCredentialRetriever =
        new DockerConfigCredentialRetriever("unknown registry",
                                            dockerConfigFile);
  ...
  public void testRetrieve_credentialFromAlias() {
    Mockito
        .when(
            mockDockerConfig.getCredentialHelperFor("registry.hub.docker.com"))
        .thenReturn(mockDockerCredentialHelper);
    DockerConfigCredentialRetriever dockerConfigCredentialRetriever =
        new DockerConfigCredentialRetriever("registry.hub.docker.com",
                                            dockerConfigFile);
  ...
  public void testRetrieve_suffixMatching()
    ...
    Path dockerConfigFile = Paths.get(
        Resources.getResource("core/json/dockerconfig_index_docker_io_v1.json")
            .toURI());

    DockerConfigCredentialRetriever dockerConfigCredentialRetriever =
        new DockerConfigCredentialRetriever("index.docker.io",
                                            dockerConfigFile);
    ...
    Assert.assertEquals(" index.docker.io/v1/",

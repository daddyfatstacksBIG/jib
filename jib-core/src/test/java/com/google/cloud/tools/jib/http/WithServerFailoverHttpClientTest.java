...
public class WithServerFailoverHttpClientTest {
  ...
  public void testRedirectionUrls()
    ...
    List<String> responses = Arrays.asList(
        redirect301, redirect302, redirect303, redirect307, redirect308, ok200);
    ...
    try (TestWebServer server = new TestWebServer(false, responses, 1)) {
      ...
      Assert.assertThat(
          server.getInputRead(),
          CoreMatchers.containsString(
              "GET /?id=301&_auth_=exp=1572285389~hmac=f0a387f0 "));
      Assert.assertThat(
          server.getInputRead(),
          CoreMatchers.containsString(
      ...
      Assert.assertThat(
          server.getInputRead(),
          CoreMatchers.containsString(
              "GET /?id=303&_auth_=exp=1572285389~hmac=f0a387f0 "));
      Assert.assertThat(
          server.getInputRead(),
          CoreMatchers.containsString(
      ...
      Assert.assertThat(
          server.getInputRead(),
          CoreMatchers.containsString(
              "GET /?id=308&_auth_=exp=1572285389~hmac=f0a387f0 "));
    }
  }
}

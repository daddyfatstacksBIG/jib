...
public class BuildContextTest {
  ...
  public void testBuilder() throws Exception {
    ...
    Set<String> additionalTargetImageTags =
        ImmutableSet.of("tag1", "tag2", "tag3");
    Set<String> expectedTargetImageTags =
        ImmutableSet.of("targettag", "tag1", "tag2", "tag3");
    List<CredentialRetriever> credentialRetrievers = Collections.singletonList(
    ...
    ImmutableSet<Port> expectedExposedPorts =
        ImmutableSet.of(Port.tcp(1000), Port.tcp(2000));
    Map<String, String> expectedLabels =
        ImmutableMap.of("key1", "value1", "key2", "value2");
    Class<? extends BuildableManifestTemplate> expectedTargetFormat =
        OciManifestTemplate.class;
    Path expectedApplicationLayersCacheDirectory =
        Paths.get("application/layers");
    ...
    ImageConfiguration baseImageConfiguration =
        ImageConfiguration
            .builder(ImageReference.of(expectedBaseImageServerUrl,
                                       expectedBaseImageName,
                                       expectedBaseImageTag))
            .build();
    ImageConfiguration targetImageConfiguration =
        ImageConfiguration
            .builder(ImageReference.of(expectedTargetServerUrl,
    ...
    Assert.assertEquals(
        ...
        buildContext.getContainerConfiguration().getCreationTime());
    Assert.assertEquals(
    ...
    Assert.assertEquals(expectedBaseImageTag,
                        buildContext.getBaseImageConfiguration().getImageTag());
    Assert.assertEquals(
    ...
    Assert.assertEquals(
        expectedTargetTag,
        buildContext.getTargetImageConfiguration().getImageTag());
    Assert.assertEquals(expectedTargetImageTags,
    ...
    Assert.assertEquals(
        ...
        buildContext.getContainerConfiguration().getProgramArguments());
    Assert.assertEquals(
        expectedEnvironment,
        buildContext.getContainerConfiguration().getEnvironmentMap());
    Assert.assertEquals(
        expectedExposedPorts,
        buildContext.getContainerConfiguration().getExposedPorts());
    Assert.assertEquals(expectedLabels,
                        buildContext.getContainerConfiguration().getLabels());
    ...
    Assert.assertEquals(expectedBaseImageLayersCacheDirectory,
                        buildContextBuilder.getBaseImageLayersCacheDirectory());
    Assert.assertEquals(expectedLayerConfigurations,
  ...
  public void testBuilder_default() throws IOException {
    ...
    Assert.assertEquals(ImmutableSet.of("targettag"),
                        buildContext.getAllTargetImageTags());
    Assert.assertEquals(V22ManifestTemplate.class,
    ...
    Assert.assertNotNull(
        buildContextBuilder.getBaseImageLayersCacheDirectory());
    Assert.assertEquals(Paths.get("ignored"),
                        buildContextBuilder.getBaseImageLayersCacheDirectory());
    ...
    try {
      BuildContext.builder()
          .setBaseImageConfiguration(
              ImageConfiguration.builder(Mockito.mock(ImageReference.class))
                  .build())
          .setBaseImageLayersCacheDirectory(Paths.get("ignored"))
          .setApplicationLayersCacheDirectory(Paths.get("ignored"))
          .build();
      ...
      Assert.assertEquals("target image configuration is required but not set",
                          ex.getMessage());
    }
  ...
  public void testBuilder_digestWarning()
    ...
    builder
        .setBaseImageConfiguration(
            ImageConfiguration
                .builder(ImageReference.parse(
                    "image@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa"))
                .build())
        .build();
    Mockito.verify(mockEventHandlers, Mockito.never())
        .dispatch(LogEvent.warn(Mockito.anyString()));

    builder
        .setBaseImageConfiguration(
            ImageConfiguration.builder(ImageReference.parse("image:tag"))
                .build())
        .build();
    Mockito.verify(mockEventHandlers)
        .dispatch(LogEvent.warn(

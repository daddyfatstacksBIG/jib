...
class BuildImageStep implements Callable<Image> {
  ...
  BuildImageStep(BuildContext buildContext,
                 ProgressEventDispatcher.Factory progressEventDispatcherFactory,
                 Image baseImage, List<PreparedLayer> baseImageLayers,
  ...
  public Image call() throws LayerPropertyNotFoundException {
    try (ProgressEventDispatcher ignored =
         ...
         TimerEventDispatcher ignored2 = new TimerEventDispatcher(
      ...
      imageBuilder.setArchitecture(baseImage.getArchitecture())
      ...
      for (int count = 0; count < baseImageLayers.size() - nonEmptyLayerCount;
           count++) {
        imageBuilder.addHistory(HistoryEntry.builder()
      ...
      for (PreparedLayer applicationLayer : applicationLayers) {
        imageBuilder.addLayer(applicationLayer)
            .addHistory(HistoryEntry.builder()
                            .setCreationTimestamp(layerCreationTime)
                            .setAuthor("Jib")
                            .setCreatedBy(buildContext.getToolName() + ":" +
                                          buildContext.getToolVersion())
      ...
      if (containerConfiguration != null) {
        ...
        if (containerConfiguration.getWorkingDirectory() != null) {
          imageBuilder.setWorkingDirectory(
              containerConfiguration.getWorkingDirectory().toString());
        }
      }
  ...
  /**
   * Computes the image entrypoint. If {@link
   * ContainerConfiguration#getEntrypoint()} is null, the entrypoint is
   * inherited from the base image. Otherwise {@link
  ...
  /**
   * Computes the image program arguments. If {@link
   * ContainerConfiguration#getEntrypoint()} and
   * {@link ContainerConfiguration#getProgramArguments()} are null, the program
   * arguments are inherited from the base image. Otherwise {@link
   * ContainerConfiguration#getProgramArguments()} is returned.
   *
   * @param baseImage the base image
   * @param containerConfiguration the container configuration
  ...
  @Nullable
  private ImmutableList<String>
  computeProgramArguments(Image baseImage,
                          ContainerConfiguration containerConfiguration) {

...
/**
 * Thread-safe HTTP client that can automatically failover from secure HTTPS to
 * insecure HTTPS or HTTP. Intended to be created once and shared to be called
 * at multiple places. Callers should close the returned {@link Response}.
 *
 ...
 *         <li>If (1) fails due to {@link SSLException}, re-attempts secure
 * HTTPS on the specified port but disabling certificate validation. <li>If (2)
 * fails again due to {@link SSLException}, attempts plain-HTTP on the specified
 *             port.
 *       </ol>
 ...
 *         <li>If (1) fails due to {@link SSLException}, re-attempts secure
 * HTTPS on port 443 but disabling certificate validation. <li>If (2) fails
 * again due to {@link SSLException}, attempts plain-HTTP on port 80 (default
 * HTTP port). <li>Or, if (1) fails due to non-timeout {@link ConnectException},
...
public class FailoverHttpClient {
  ...
  private static HttpTransport getSecureHttpTransport() {
    // Do not use NetHttpTransport. It does not process response errors
    // properly. See https://github.com/google/google-http-java-client/issues/39
    //
    // A new ApacheHttpTransport needs to be created for each connection because
    // otherwise HTTP connection persistence causes the connection to throw
    ...
    try {
      HttpClientBuilder httpClientBuilder =
          ApacheHttpTransport.newDefaultHttpClientBuilder()
              .setSSLSocketFactory(
                  null) // creates new factory with the SSLContext given below
              .setSSLContext(SslUtils.trustAllSSLContext())
              .setSSLHostnameVerifier(new NoopHostnameVerifier());
      // Do not use NetHttpTransport. See comments in getConnectionFactory for
      // details.
  ...
  public FailoverHttpClient(boolean enableHttpAndInsecureFailover,
    ...
    this(enableHttpAndInsecureFailover, sendAuthorizationOverHttp, logger,
  ...
  /**
   * Closes all connections and allocated resources, whether they are currently
   * used or not.
   *
   * <p>If an I/O error occurs, shutdown attempts stop immediately, resulting in
   * partial resource release up to that point. The method can be called again
   * later to re-attempt releasing all resources.
   *
  ...
  public Response call(String httpMethod, URL url, Request request)
    ...
    if (!isHttpsProtocol(url)) {
      if (enableHttpAndInsecureFailover) { // HTTP requested. We only care if
                                           // HTTP is enabled.
    ...
    Optional<Response> fastPathResponse =
        followFailoverHistory(httpMethod, url, request);
    ...
    try {
      ...
      try {
        ...
        Response response =
            call(httpMethod, url, request, getHttpTransport(false));
        failoverHistory.put(url.getHost() + ":" + url.getPort(),
                            Failover.INSECURE_HTTPS);
        return response;

      } catch (SSLException
                   ignored) { // This is usually when the server is plain-HTTP.
        logHttpFailover(url);
        Response response =
            call(httpMethod, toHttp(url), request, getHttpTransport(true));
      ...
      if (ex.getMessage() == null || !ex.getMessage().contains("timed out")) {
        ...
        if (enableHttpAndInsecureFailover && isHttpsProtocol(url) &&
            url.getPort() == -1) {
          logHttpFailover(url);
          Response response =
              call(httpMethod, toHttp(url), request, getHttpTransport(true));
  ...
  private Optional<Response> followFailoverHistory(String httpMethod, URL url,
                                                   Request request)
      throws IOException {
    Preconditions.checkArgument(isHttpsProtocol(url));
    switch (failoverHistory.getOrDefault(url.getHost() + ":" + url.getPort(),
                                         Failover.NONE)) {
    case HTTP:
      return Optional.of(
          call(httpMethod, toHttp(url), request, getHttpTransport(true)));
    ...
    default:
      return Optional
          .empty(); // No history found. Should go for normal execution path.
    }
  }

  private Response call(String httpMethod, URL url, Request request,
                        HttpTransport httpTransport) throws IOException {
    boolean clearAuthorization =
        !isHttpsProtocol(url) && !sendAuthorizationOverHttp;

    HttpHeaders requestHeaders =
        clearAuthorization ? request.getHeaders().clone().setAuthorization(
                                 (String)null) // deep clone implemented
                           : request.getHeaders();

    HttpRequest httpRequest = httpTransport.createRequestFactory()
                                  .buildRequest(httpMethod, new GenericUrl(url),
                                                request.getHttpContent())
    ...
    try {
      ...
      synchronized (responsesCreated) { responsesCreated.add(response); }
      return response;
  ...
  private HttpTransport getHttpTransport(boolean secureTransport) {
    HttpTransport transport = secureTransport
                                  ? secureHttpTransportFactory.get()
                                  : insecureHttpTransportFactory.get();
    synchronized (transportsCreated) { transportsCreated.add(transport); }
    return transport;
  }

  private void logHttpFailover(URL url) {
    String log = "Failed to connect to " + url +
                 " over HTTPS. Attempting again with HTTP.";
    logger.accept(LogEvent.warn(log));
  }

  private void logInsecureHttpsFailover(URL url) {
    String log = "Cannot verify server at " + url +

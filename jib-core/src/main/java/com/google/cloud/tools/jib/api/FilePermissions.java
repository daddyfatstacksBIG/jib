...
public class FilePermissions {
  ...
  public static final FilePermissions DEFAULT_FILE_PERMISSIONS =
      new FilePermissions(0644);

  /** Default permissions for folders added to the container. */
  public static final FilePermissions DEFAULT_FOLDER_PERMISSIONS =
      new FilePermissions(0755);

  /**
   * Matches an octal string representation of file permissions. From left to
   * right, each digit represents permissions for owner, group, and other.
   */
  private static final String OCTAL_PATTERN = "[0-7][0-7][0-7]";

  /**
   * Maps from a {@link PosixFilePermission} to its corresponding file
   * permission bit.
   */
  private static final ImmutableMap<PosixFilePermission, Integer>
      PERMISSION_MAP = ImmutableMap.<PosixFilePermission, Integer>builder()
  ...
  /**
   * Creates a new {@link FilePermissions} from an octal string representation
   * (e.g. "123", "644", "755", etc).
   *
  ...
  /**
   * Creates a new {@link FilePermissions} from a set of {@link
   * PosixFilePermission}.
   *
  ...
  fromPosixFilePermissions(Set<PosixFilePermission> posixFilePermissions) {
    ...
    for (PosixFilePermission permission : posixFilePermissions) {
      permissionBits |=
          Preconditions.checkNotNull(PERMISSION_MAP.get(permission));
    }
  ...
  /**
   * Gets the corresponding permissions bits specified by the {@link
   * FilePermissions}.
   *
   * @return the permission bits
   */
  public int getPermissionBits() { return permissionBits; }
   ...
   */
  public String toOctalString() { return Integer.toString(permissionBits, 8); }

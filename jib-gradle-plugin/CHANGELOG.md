...
- Support for local base images by prefixing `jib.from.image` with `docker://`
  to build from a docker daemon image, or `tar://` to build from a tarball image
...
- Can now set container creation timestamp with `jib.container.creationTime`.
  The value should be `EPOCH`, `USE_CURRENT_TIMESTAMP`, or an ISO 8601 date time
...
- `container.appRoot` configuration parameter to configure app root in the image
  ([#984](https://github.com/GoogleContainerTools/jib/pull/984))
...
- Export a Docker context (including a Dockerfile) with `jibExportDockerContext`
  ([#204](https://github.com/google/jib/issues/204))

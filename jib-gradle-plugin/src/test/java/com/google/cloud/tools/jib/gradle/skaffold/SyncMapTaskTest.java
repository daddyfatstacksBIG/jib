...
public class SyncMapTaskTest {
  ...
  @ClassRule
  public static final TestProject warProject = new TestProject("war_servlet25");
   ...
   */
  private static SkaffoldSyncMapTemplate
  generateTemplate(TestProject project, @Nullable String moduleName,
                   @Nullable List<String> params) throws IOException {
    String taskName = ":" + (moduleName == null ? "" : moduleName + ":") +
                      JibPlugin.SKAFFOLD_SYNC_MAP_TASK_NAME;
    ...
    List<String> outputLines = Splitter.on(System.lineSeparator())
                                   .omitEmptyStrings()
  ...
  private static void assertFilePaths(Path src, AbsoluteUnixPath dest,
  ...
  public void testSyncMapTask_singleProject() throws IOException {
    ...
    SkaffoldSyncMapTemplate parsed =
        generateTemplate(simpleTestProject, null, null);

    List<FileTemplate> generated = parsed.getGenerated();
    Assert.assertEquals(2, generated.size());
    assertFilePaths(projectRoot.resolve("build/resources/main/world"),
                    AbsoluteUnixPath.get("/app/resources/world"),
                    generated.get(0));
    assertFilePaths(
        projectRoot.resolve(
            "build/classes/java/main/com/test/HelloWorld.class"),
    ...
    assertFilePaths(projectRoot.resolve("src/main/custom-extra-dir/bar/cat"),
                    AbsoluteUnixPath.get("/bar/cat"), direct.get(0));
    assertFilePaths(projectRoot.resolve("src/main/custom-extra-dir/foo"),
                    AbsoluteUnixPath.get("/foo"), direct.get(1));
  }
  ...
  public void testSyncMapTask_multiProjectOutput() throws IOException {
    ...
    SkaffoldSyncMapTemplate parsed =
        generateTemplate(multiTestProject, "complex-service", null);

    List<FileTemplate> generated = parsed.getGenerated();
    Assert.assertEquals(4, generated.size());
    assertFilePaths(libRoot.resolve("build/libs/lib.jar"),
                    AbsoluteUnixPath.get("/app/libs/lib.jar"),
                    generated.get(0));
    assertFilePaths(
        complexServiceRoot.resolve("build/resources/main/resource1.txt"),
        AbsoluteUnixPath.get("/app/resources/resource1.txt"), generated.get(1));
    assertFilePaths(
        complexServiceRoot.resolve("build/resources/main/resource2.txt"),
        AbsoluteUnixPath.get("/app/resources/resource2.txt"), generated.get(2));
    assertFilePaths(
    ...
    assertFilePaths(
        ...
        direct.get(0));
    assertFilePaths(complexServiceRoot.resolve("src/main/other-jib/extra-file"),
                    AbsoluteUnixPath.get("/extra-file"), direct.get(1));
  }
  ...
  public void testSyncMapMojo_failIfWar() throws IOException {
    ...
    try {
      ...
      Assert.assertTrue(ex.getMessage().contains(
          ...
          projectRoot.getFileName() + " is configured to generate a 'war'"));
    }
  }

  @Test
  public void testSyncMapMojo_failIfJarContainerizationMode()
      throws IOException {
    Path projectRoot = simpleTestProject.getProjectRoot();
    try {
      generateTemplate(simpleTestProject, null,
                       ImmutableList.of("-Djib.containerizingMode=packaged"));
      Assert.fail();
    } catch (UnexpectedBuildFailure ex) {
      Assert.assertTrue(ex.getMessage().contains(
          ...
          projectRoot.getFileName() + " is 'packaged'"));
    }
  }
}

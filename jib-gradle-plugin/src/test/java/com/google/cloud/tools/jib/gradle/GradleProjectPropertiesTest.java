...
public class GradleProjectPropertiesTest {
  ...
  private static class ContainerBuilderLayers {
    ...
    private ContainerBuilderLayers(BuildContext buildContext) {
      ...
      classesLayerEntries = getLayerConfigurationsByName(
          buildContext, LayerType.CLASSES.getName());
  ...
  @Mock(answer = Answers.RETURNS_DEEP_STUBS) private Project mockProject;
  ...
  public void setUp() throws URISyntaxException, IOException {
    ...
    Path emptyDirectory =
        getResource("gradle/webapp").resolve("WEB-INF/classes/empty_dir");
    Files.createDirectories(emptyDirectory);

    gradleProjectProperties = new GradleProjectProperties(
        mockProject, mockLogger, mockTempDirectoryProvider);
  }
  ...
  public void testIsWarProject() {
    Mockito.when(mockProject.getPlugins().hasPlugin(WarPlugin.class))
        .thenReturn(true);
  ...
  public void testConvertPermissionsMap() {
    Assert.assertEquals(
        ImmutableMap.of("/test/folder/file1",
                        FilePermissions.fromOctalString("123"), "/test/file2",
  ...
  public void testCreateContainerBuilder_correctFiles()
      throws URISyntaxException, InvalidImageReferenceException,
  ...
  public void testCreateContainerBuilder_defaultAppRoot()
    ...
    BuildContext buildContext =
        setupBuildContext(JavaContainerBuilder.DEFAULT_APP_ROOT);
    ContainerBuilderLayers layers = new ContainerBuilderLayers(buildContext);
    assertExtractionPathsUnordered(
  ...
  public void testCreateContainerBuilder_defaultWebAppRoot()
    ...
    BuildContext buildContext =
        setupBuildContext(JavaContainerBuilder.DEFAULT_WEB_APP_ROOT);
    ContainerBuilderLayers layers = new ContainerBuilderLayers(buildContext);
    assertSourcePathsUnordered(
        ImmutableList.of(
            unzipTarget.resolve("WEB-INF/lib/dependency-1.0.0.jar")),
  ...
  public void testGetWarFilePath() {
    Mockito.when(mockProject.getPlugins().hasPlugin(WarPlugin.class))
        .thenReturn(true);
    Mockito.when(mockTaskContainer.named("war"))
        .thenReturn(mockWarTaskProvider);
    Mockito.when(mockWarTaskProvider.get().getOutputs().getFiles().getAsPath())
        .thenReturn("/war/file/here.war");

    Assert.assertEquals("/war/file/here.war",
                        gradleProjectProperties.getWarFilePath());
  }

  @Test
  public void testGetWarFilePath_bootWar() {
    Mockito.when(mockProject.getPlugins().hasPlugin("org.springframework.boot"))
        .thenReturn(true);
    Mockito.when(mockTaskContainer.named("bootWar"))
        .thenReturn(mockBootWarTaskProvider);
    Mockito.when(mockBootWarTaskProvider.get().getEnabled()).thenReturn(true);
    Mockito
        .when(mockBootWarTaskProvider.get().getOutputs().getFiles().getAsPath())
        .thenReturn("/boot/war/file.war");

    Assert.assertEquals("/boot/war/file.war",
                        gradleProjectProperties.getWarFilePath());
  }
  ...
  private Path setUpWarProject(Path webAppDirectory) throws IOException {
    Path targetZip = zipUpDirectory(
        webAppDirectory,
        temporaryFolder.getRoot().toPath().resolve("my-app.war"));

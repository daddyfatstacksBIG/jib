...
public class JibExtensionTest {
  ...
  public void setUp() {
    ...
    testJibExtension = fakeProject.getExtensions().create(
        JibPlugin.JIB_EXTENSION_NAME, JibExtension.class, fakeProject);
  ...
  public void testFrom() {
    ...
    Assert.assertEquals("some cred helper",
                        testJibExtension.getFrom().getCredHelper());
    Assert.assertEquals("some username",
                        testJibExtension.getFrom().getAuth().getUsername());
    Assert.assertEquals("some password",
                        testJibExtension.getFrom().getAuth().getPassword());
  }
  ...
  public void testTo() {
    ...
    Assert.assertEquals("some cred helper",
                        testJibExtension.getTo().getCredHelper());
    Assert.assertEquals("some username",
                        testJibExtension.getTo().getAuth().getUsername());
    Assert.assertEquals("some password",
                        testJibExtension.getTo().getAuth().getPassword());
  }

  @Test
  public void testContainer() {
    Assert.assertEquals(Collections.emptyList(),
                        testJibExtension.getContainer().getJvmFlags());
    Assert.assertEquals(Collections.emptyMap(),
                        testJibExtension.getContainer().getEnvironment());
    Assert.assertEquals(Collections.emptyList(),
                        testJibExtension.getContainer().getExtraClasspath());
    Assert.assertNull(testJibExtension.getContainer().getMainClass());
    Assert.assertNull(testJibExtension.getContainer().getArgs());
    Assert.assertSame(ImageFormat.Docker,
                      testJibExtension.getContainer().getFormat());
    Assert.assertEquals(Collections.emptyList(),
                        testJibExtension.getContainer().getPorts());
    Assert.assertEquals(Collections.emptyMap(),
    ...
    Assert.assertEquals("EPOCH",
                        testJibExtension.getContainer().getCreationTime());

    testJibExtension.container(container -> {
    ...
    Assert.assertEquals(Arrays.asList("jvmFlag1", "jvmFlag2"),
                        container.getJvmFlags());
    Assert.assertEquals(ImmutableMap.of("var1", "value1", "var2", "value2"),
                        container.getEnvironment());
    Assert.assertEquals(ImmutableList.of("/d1", "/d2", "/d3"),
                        container.getExtraClasspath());
    Assert.assertEquals("mainClass",
                        testJibExtension.getContainer().getMainClass());
    Assert.assertEquals(Arrays.asList("arg1", "arg2", "arg3"),
                        container.getArgs());
    Assert.assertEquals(Arrays.asList("1000", "2000-2010", "3000"),
                        container.getPorts());
    Assert.assertEquals(ImmutableMap.of("label1", "value1", "label2", "value2"),
                        container.getLabels());
    Assert.assertSame(ImageFormat.OCI, container.getFormat());
    Assert.assertEquals("some invalid appRoot value", container.getAppRoot());
    Assert.assertEquals("some invalid time value",
                        container.getFilesModificationTime());
  }
  ...
  public void testExtraDirectories_default() {
    Assert.assertEquals(
        Arrays.asList(Paths.get(fakeProject.getProjectDir().getPath(), "src",
                                "main", "jib")),
  ...
  public void testExtraDirectories() {
    testJibExtension.extraDirectories(extraDirectories -> {
      ...
      extraDirectories.setPermissions(
  ...
  public void testExtraDirectories_stringListForPaths() {
    ...
    Assert.assertEquals(
        Arrays.asList(Paths.get(projectRoot, "test", "path"),
                      Paths.get(projectRoot, "another", "path")),
        testJibExtension.getExtraDirectories().getPaths());
  }
  ...
  public void testExtraDirectories_fileListForPaths() {
    testJibExtension.extraDirectories(extraDirectories -> {
      extraDirectories.setPaths(
          Arrays.asList(Paths.get("test", "path").toFile(),
                        Paths.get("another", "path").toFile()));
    });
  ...
  public void testDockerClient() {
    testJibExtension.dockerClient(dockerClient -> {
      dockerClient.setExecutable("test-executable");
      dockerClient.setEnvironment(
          ImmutableMap.of("key1", "val1", "key2", "val2"));
    });

    Assert.assertEquals(Paths.get("test-executable"),
                        testJibExtension.getDockerClient().getExecutablePath());
    Assert.assertEquals(ImmutableMap.of("key1", "val1", "key2", "val2"),
                        testJibExtension.getDockerClient().getEnvironment());
  }

  @Test
  public void testOutputFiles() {
    testJibExtension.outputPaths(outputFiles -> {
      outputFiles.setDigest("/path/to/digest");
      outputFiles.setImageId("/path/to/id");
      outputFiles.setTar("path/to/tar");
    });

    Assert.assertEquals(Paths.get("/path/to/digest").toAbsolutePath(),
                        testJibExtension.getOutputPaths().getDigestPath());
    Assert.assertEquals(Paths.get("/path/to/id").toAbsolutePath(),
  ...
  public void testProperties() {
    ...
    Assert.assertEquals("credHelper",
                        testJibExtension.getFrom().getCredHelper());
    ...
    Assert.assertEquals(ImmutableSet.of("tag1", "tag2", "tag3"),
    ...
    Assert.assertEquals(ImmutableList.of("port1", "port2", "port3"),
                        testJibExtension.getContainer().getPorts());
    ...
    System.setProperty("jib.container.filesModificationTime",
                       "2011-12-03T22:42:05Z");
    Assert.assertEquals(
        "2011-12-03T22:42:05Z",
        testJibExtension.getContainer().getFilesModificationTime());
    ...
    Assert.assertEquals(Arrays.asList(Paths.get("/foo"), Paths.get("/bar/baz")),
                        testJibExtension.getExtraDirectories().getPaths());
    System.setProperty("jib.extraDirectories.permissions",
                       "/foo/bar=707,/baz=456");
    Assert.assertEquals(
    ...
    Assert.assertEquals(Paths.get("test-exec"),
                        testJibExtension.getDockerClient().getExecutablePath());
    ...
    Assert.assertEquals(Paths.get("/digest/path").toAbsolutePath(),
                        testJibExtension.getOutputPaths().getDigestPath());
    System.setProperty("jib.outputPaths.imageId", "/id/path");
    Assert.assertEquals(Paths.get("/id/path").toAbsolutePath(),
                        testJibExtension.getOutputPaths().getImageIdPath());
    System.setProperty("jib.outputPaths.tar", "/tar/path");
    Assert.assertEquals(Paths.get("/tar/path").toAbsolutePath(),
                        testJibExtension.getOutputPaths().getTarPath());
    // Relative paths
    System.setProperty("jib.outputPaths.digest", "digest/path");
    Assert.assertEquals(

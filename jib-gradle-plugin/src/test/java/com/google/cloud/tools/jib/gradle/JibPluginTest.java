...
public class JibPluginTest {
  ...
  public void testCheckJibVersionNames() {
    ...
    Assert.assertEquals(JibPlugin.REQUIRED_VERSION_PROPERTY_NAME,
                        "jib.requiredVersion");
    Assert.assertEquals(JibPlugin.CHECK_REQUIRED_VERSION_TASK_NAME,
  ...
  public void testProjectDependencyAssembleTasksAreRun() {
    ...
    KNOWN_JIB_TASKS.forEach(
        taskName
        -> rootProject.getTasks().getByPath(taskName).dependsOn(
            dependencyTask));
  ...
  public void testWebAppProject() {
    ...
    Task explodedWarTask =
        tasks.getByPath(":" + JibPlugin.EXPLODED_WAR_TASK_NAME);
    Assert.assertNotNull(explodedWarTask);
    Assert.assertEquals(
        rootProject.getBuildDir().toPath().resolve(
            ProjectProperties.EXPLODED_WAR_DIRECTORY_NAME),
        ((ExplodedWarTask)explodedWarTask).getExplodedWarDirectory().toPath());

    Assert.assertEquals(
        explodedWarTask,
        ((TaskProvider<Task>)tasks.getByPath(JibPlugin.BUILD_IMAGE_TASK_NAME)
             .getDependsOn()
             .iterator()
             .next())
            .get());
    Assert.assertEquals(
        explodedWarTask,
        ((TaskProvider<Task>)tasks.getByPath(JibPlugin.BUILD_DOCKER_TASK_NAME)
             .getDependsOn()
             .iterator()
             .next())
            .get());
    Assert.assertEquals(
        explodedWarTask,
        ((TaskProvider<Task>)tasks.getByPath(JibPlugin.BUILD_TAR_TASK_NAME)
             .getDependsOn()
             .iterator()
             .next())
            .get());

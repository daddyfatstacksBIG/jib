...
public class JibPluginTest {
  ...
  private static final ImmutableList<String> KNOWN_JIB_TASKS = ImmutableList.of(
      JibPlugin.BUILD_IMAGE_TASK_NAME, JibPlugin.BUILD_DOCKER_TASK_NAME,
  ...
  public void testCheckJibVersionNames() {
    ...
    Assert.assertEquals(JibPlugin.REQUIRED_VERSION_PROPERTY_NAME,
                        "jib.requiredVersion");
    Assert.assertEquals(JibPlugin.SKAFFOLD_CHECK_REQUIRED_VERSION_TASK_NAME,
  ...
  public void testCheckJibVersionInvoked() {
    ...
    System.setProperty(JibPlugin.REQUIRED_VERSION_PROPERTY_NAME,
                       "10000.0"); // not here yet
    try {
      project.getPluginManager().apply("com.google.cloud.tools.jib");
      Assert.fail("should have failed");
    } catch (GradleException ex) {
      // Gradle tests aren't run from a jar and so don't have an identifiable
      // plugin version
      Assert.assertEquals(
          "Failed to apply plugin [id 'com.google.cloud.tools.jib']",
          ex.getMessage());
      Assert.assertEquals("Could not determine Jib plugin version",
  ...
  public void testProjectDependencyAssembleTasksAreRun() {
    ...
    // our service DOES depend on this, and jib should trigger an assemble from
    // this project
    ...
    // our service doesn't depend on this, and jib should NOT trigger an
    // assemble from this project
    Project unrelatedSubProject = ProjectBuilder.builder()
                                      .withParent(rootProject)
                                      .withProjectDir(testProjectRoot.getRoot())
                                      .withName("unrelated")
                                      .build();
    ...
    rootProject.getConfigurations().getByName("compile").getDependencies().add(
        rootProject.getDependencies().project(
            ImmutableMap.of("path", subProject.getPath())));

    // programmatic check
    Assert.assertEquals(Collections.singletonList(":sub"),
    ...
    // add a custom task that our jib tasks depend on to ensure we do not
    // overwrite this dependsOn
    TaskProvider<Task> dependencyTask =
        rootProject.getTasks().register("myCustomTask", task -> {});
    KNOWN_JIB_TASKS.forEach(
        taskName -> tasks.getByPath(taskName).dependsOn(dependencyTask));
  ...
  public void testWebAppProject() {
    Project project =
        createProject("java", "war", "com.google.cloud.tools.jib");
    ...
    for (String taskName : KNOWN_JIB_TASKS) {
      List<TaskProvider<?>> taskProviders =
          (List<TaskProvider<?>>)tasks.getByPath(taskName)
              .getDependsOn()
              .iterator()
              .next();
  ...
  public void testWebAppProject_bootWar() {
    Project project = createProject("java", "war", "org.springframework.boot",
    ...
    for (String taskName : KNOWN_JIB_TASKS) {
      List<TaskProvider<?>> taskProviders =
          (List<TaskProvider<?>>)tasks.getByPath(taskName)
              ...
              .iterator()
              .next();
      Assert.assertEquals(ImmutableSet.of(warTask, bootWarTask),
                          taskProviders.stream()
  ...
  public void testSpringBootJarProject_nonPackagedMode() {
    Project project = createProject("java", "org.springframework.boot",
                                    "com.google.cloud.tools.jib");
  ...
  public void testSpringBootJarProject_packagedMode() {
    ...
    JibExtension jibExtension =
        (JibExtension)project.getExtensions().getByName("jib");
  ...
  public void testJibTaskGroupIsSet() {
    ...
    KNOWN_JIB_TASKS.forEach(
        taskName
        -> Assert.assertEquals(taskName, "Jib",
                               tasks.getByPath(taskName).getGroup()));
  }

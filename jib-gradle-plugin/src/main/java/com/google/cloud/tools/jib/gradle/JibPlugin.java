...
public class JibPlugin implements Plugin<Project> {
  ...
  @VisibleForTesting
  static final GradleVersion GRADLE_MIN_VERSION = GradleVersion.version("5.1");
  ...
  public static final String SKAFFOLD_FILES_TASK_V2_NAME =
      "_jibSkaffoldFilesV2";
  public static final String SKAFFOLD_INIT_TASK_NAME = "_jibSkaffoldInit";
  public static final String SKAFFOLD_SYNC_MAP_TASK_NAME =
      "_jibSkaffoldSyncMap";
  ...
  public void apply(Project project) {
    ...
    tasks.register(SKAFFOLD_FILES_TASK_V2_NAME, FilesTaskV2.class)
        .configure(task -> task.setJibExtension(jibExtension));
    tasks.register(SKAFFOLD_INIT_TASK_NAME, InitTask.class)
        .configure(task -> task.setJibExtension(jibExtension));
    TaskProvider<SyncMapTask> syncMapTask =
        tasks.register(SKAFFOLD_SYNC_MAP_TASK_NAME, SyncMapTask.class,
    ...
    tasks.register(SKAFFOLD_CHECK_REQUIRED_VERSION_TASK_NAME,
                   CheckJibVersionTask.class);

    project.afterEvaluate(projectAfterEvaluation -> {
      try {
        TaskProvider<Task> warTask =
            TaskCommon.getWarTaskProvider(projectAfterEvaluation);
        ...
        if (warTask != null || bootWarTask != null) {
          ...
          TaskProvider<Task> jarTask =
              projectAfterEvaluation.getTasks().named("jar");
          dependsOnTask.add(jarTask);

          if (projectAfterEvaluation.getPlugins().hasPlugin(

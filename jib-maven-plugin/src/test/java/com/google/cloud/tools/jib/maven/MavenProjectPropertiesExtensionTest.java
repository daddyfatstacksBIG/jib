...
public class MavenProjectPropertiesExtensionTest {
  ...
  private static class FooExtension implements JibMavenPluginExtension {
    ...
    extendContainerBuildPlan(ContainerBuildPlan buildPlan,
                             Map<String, String> properties,
                             MavenData mavenData, ExtensionLogger logger)
        throws JibPluginExtensionException {
      return extension.extendContainerBuildPlan(buildPlan, properties,
                                                mavenData, logger);
    }
  }
  ...
  private static class BarExtensionConfig extends FooExtensionConfig {
    ...
    private BarExtensionConfig() { super(BarExtension.class.getName()); }
  }
  ...
  public void testRunPluginExtensions_noExtensionsConfigured()
    ...
    JibMavenPluginExtension extension =
        (buildPlan, properties, mavenData, logger) -> buildPlan;

    JibContainerBuilder extendedBuilder =
        mavenProjectProperties.runPluginExtensions(Arrays.asList(extension),
    ...
    Mockito.verify(mockLog).debug(
        "No Jib plugin extensions configured to load");
  }
  ...
  public void testRunPluginExtensions_configuredExtensionNotFound() {
    try {
      mavenProjectProperties.runPluginExtensions(
          Collections.emptyList(), Arrays.asList(new FooExtensionConfig()),
          containerBuilder);
      Assert.fail();
  ...
  public void testRunPluginExtensions() throws JibPluginExtensionException {
    FooExtension extension =
        new FooExtension((buildPlan, properties, mavenData, logger) -> {
    ...
    JibContainerBuilder extendedBuilder =
        mavenProjectProperties.runPluginExtensions(
            Arrays.asList(extension), Arrays.asList(new FooExtensionConfig()),
            containerBuilder);
    Assert.assertEquals("user from extension",
                        extendedBuilder.toContainerBuildPlan().getUser());

    mavenProjectProperties.waitForLoggingThread();
    Mockito.verify(mockLog).error("awesome error from my extension");
    Mockito.verify(mockLog).info(Mockito.startsWith(
  ...
  public void testRunPluginExtensions_invalidBaseImageFromExtension() {
    FooExtension extension = new FooExtension(
        (buildPlan, properties, mavenData,
         logger) -> buildPlan.toBuilder().setBaseImage(" in*val+id").build());
    ...
    try {
      ...
      Assert.assertEquals("invalid base image reference:  in*val+id",
                          ex.getMessage());
      Assert.assertThat(
          ex.getCause(),
          CoreMatchers.instanceOf(InvalidImageReferenceException.class));
    }
  }
  ...
  public void testRunPluginExtensions_extensionOrder()
    ...
    FooExtension fooExtension = new FooExtension(
        (buildPlan, properties, mavenData,
         logger) -> buildPlan.toBuilder().setBaseImage("foo").build());
    BarExtension barExtension = new BarExtension(
        (buildPlan, properties, mavenData,
         logger) -> buildPlan.toBuilder().setBaseImage("bar").build());
    List<JibMavenPluginExtension> extensions =
        Arrays.asList(fooExtension, barExtension);
    ...
    Assert.assertEquals("bar",
                        extendedBuilder1.toContainerBuildPlan().getBaseImage());
    ...
    Assert.assertEquals("foo",
                        extendedBuilder2.toContainerBuildPlan().getBaseImage());
  }

  @Test
  public void testRunPluginExtensions_customProperties()
      throws JibPluginExtensionException {
    FooExtension extension = new FooExtension(
    ...
    JibContainerBuilder extendedBuilder =
        mavenProjectProperties.runPluginExtensions(
            ...
            Arrays.asList(
                new FooExtensionConfig(ImmutableMap.of("user", "65432"))),
            containerBuilder);
    Assert.assertEquals("65432",
                        extendedBuilder.toContainerBuildPlan().getUser());
  }
}

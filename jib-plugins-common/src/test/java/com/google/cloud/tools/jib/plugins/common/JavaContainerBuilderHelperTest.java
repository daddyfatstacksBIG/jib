...
public class JavaContainerBuilderHelperTest {
  ...
  @Test
  public void testExtraDirectoryLayerConfiguration()
    ...
    FileEntriesLayer layerConfiguration =
        JavaContainerBuilderHelper.extraDirectoryLayerConfiguration(
            extraFilesDirectory, AbsoluteUnixPath.get("/"),
            Collections.emptyMap(), (ignored1, ignored2) -> Instant.EPOCH);
    assertSourcePathsUnordered(
        Arrays.asList(extraFilesDirectory.resolve("a"),
  ...
  public void testExtraDirectoryLayerConfiguration_globPermissions()
    ...
    Map<String, FilePermissions> permissionsMap =
        ImmutableMap.of("/a", FilePermissions.fromOctalString("123"), "/a/*",
                        FilePermissions.fromOctalString("456"), "**/bar",
                        FilePermissions.fromOctalString("765"));
    FileEntriesLayer fileEntriesLayer =
        JavaContainerBuilderHelper.extraDirectoryLayerConfiguration(
            extraFilesDirectory, AbsoluteUnixPath.get("/"), permissionsMap,
    ...
    Map<AbsoluteUnixPath, FilePermissions> expectedPermissions =
        ImmutableMap.<AbsoluteUnixPath, FilePermissions>builder()
            .put(AbsoluteUnixPath.get("/a"),
                 FilePermissions.fromOctalString("123"))
            .put(AbsoluteUnixPath.get("/a/b"),
                 FilePermissions.fromOctalString("456"))
            .put(AbsoluteUnixPath.get("/a/b/bar"),
                 FilePermissions.fromOctalString("765"))
            .put(AbsoluteUnixPath.get("/c"),
                 FilePermissions.DEFAULT_FOLDER_PERMISSIONS)
            .put(AbsoluteUnixPath.get("/c/cat"),
                 FilePermissions.DEFAULT_FILE_PERMISSIONS)
            .put(AbsoluteUnixPath.get("/foo"),
                 FilePermissions.DEFAULT_FILE_PERMISSIONS)
            .build();
    for (FileEntry entry : fileEntriesLayer.getEntries()) {
      Assert.assertEquals(expectedPermissions.get(entry.getExtractionPath()),
  ...
  public void testExtraDirectoryLayerConfiguration_overlappingPermissions()
    ...
    Path extraFilesDirectory =
        Paths.get(Resources.getResource("core/layer").toURI());
    Map<String, FilePermissions> permissionsMap =
        ImmutableMap.of("/a**", FilePermissions.fromOctalString("123"),
                        // Should be ignored, since first match takes priority
                        "/a/b**", FilePermissions.fromOctalString("000"),
                        // Should override first match since explicit path is
  ...
  public void testFromExplodedWar()
    ...
    FileOperations.copy(ImmutableList.of(resourceExplodedWar),
                        temporaryFolder.getRoot().toPath());
    Path temporaryExplodedWar =
        temporaryFolder.getRoot().toPath().resolve("exploded-war");
    ...
    JibContainerBuilder jibContainerBuilder =
        JavaContainerBuilderHelper.fromExplodedWar(javaContainerBuilder,
                                                   temporaryExplodedWar);
    BuildContext buildContext = JibContainerBuilderTestHelper.toBuildContext(
        jibContainerBuilder,
        Containerizer.to(RegistryImage.named("target"))
            .setExecutorService(MoreExecutors.newDirectExecutorService()));

    List<FileEntriesLayer> resourcesLayerConfigurations =
        getLayerConfigurationsByName(buildContext,
    ...
    List<FileEntriesLayer> dependenciesLayerConfigurations =
        getLayerConfigurationsByName(buildContext,
                                     LayerType.DEPENDENCIES.getName());
    List<FileEntriesLayer> snapshotsLayerConfigurations =
        getLayerConfigurationsByName(buildContext,
                                     LayerType.SNAPSHOT_DEPENDENCIES.getName());

    assertSourcePathsUnordered(
        Collections.singletonList(
            temporaryExplodedWar.resolve("WEB-INF/lib/dependency-1.0.0.jar")),
        dependenciesLayerConfigurations.get(0).getEntries());
    assertSourcePathsUnordered(
        Collections.singletonList(temporaryExplodedWar.resolve(
            "WEB-INF/lib/dependencyX-1.0.0-SNAPSHOT.jar")),
        snapshotsLayerConfigurations.get(0).getEntries());
    assertSourcePathsUnordered(
        Arrays.asList(temporaryExplodedWar.resolve("META-INF"),
    ...
    assertSourcePathsUnordered(
        Arrays.asList(
            ...
            temporaryExplodedWar.resolve("WEB-INF/classes/package"),
            temporaryExplodedWar.resolve(
                "WEB-INF/classes/package/Other.class")),

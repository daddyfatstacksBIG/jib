...
public class SkaffoldSyncMapTemplateTest {
  ...
  private static final String TEST_JSON =
      "{\"generated\":[{\"src\":\"" + getPathForJson(GEN_SRC) +
      "\",\"dest\":\"/genDest\"}],\"direct\":[{\"src\":\"" +
      getPathForJson(DIR_SRC_1) + "\",\"dest\":\"/dirDest1\"},{\"src\":\"" +
      getPathForJson(DIR_SRC_2) + "\",\"dest\":\"/dirDest2\"}]}";
  private static final String TEST_JSON_EMPTY_GENERATED =
      "{\"generated\":[],\"direct\":[{\"src\":\"" + getPathForJson(DIR_SRC_1) +
      "\",\"dest\":\"/dirDest1\"},{\"src\":\"" + getPathForJson(DIR_SRC_2) +
      "\",\"dest\":\"/dirDest2\"}]}";
  private static final String TEST_JSON_NO_GENERATED =
      "{\"direct\":[{\"src\":\"" + getPathForJson(DIR_SRC_1) +
      "\",\"dest\":\"/dirDest1\"},{\"src\":\"" + getPathForJson(DIR_SRC_2) +
      "\",\"dest\":\"/dirDest2\"}]}";
  ...
  // manually correct "\" that we inject into the strings above for windows
  // paths, this is only needed for this test, when json writes the string out
  // in the actual code, it does the right thing
  ...
  public void testFrom_badPropertyName() throws IOException {
    try {
      ...
      Assert.assertTrue(
          ex.getMessage().contains("Unrecognized field \"jean-luc\""));
    }
  }
  ...
  public void testFrom_missingField() throws IOException {
    try {
      ...
      Assert.assertTrue(
          ex.getMessage().contains("Missing required creator property 'dest'"));
    }
  }
  ...
  public void testGetJsonString() throws IOException {
    ...
    ssmt.addGenerated(
        new LayerEntry(GEN_SRC, AbsoluteUnixPath.get("/genDest"),
                       FilePermissions.DEFAULT_FILE_PERMISSIONS,
                       LayerConfiguration.DEFAULT_MODIFICATION_TIME));
    ssmt.addDirect(
        new LayerEntry(DIR_SRC_1, AbsoluteUnixPath.get("/dirDest1"),
                       FilePermissions.DEFAULT_FILE_PERMISSIONS,
                       LayerConfiguration.DEFAULT_MODIFICATION_TIME));
    ssmt.addDirect(
        new LayerEntry(DIR_SRC_2, AbsoluteUnixPath.get("/dirDest2"),
                       FilePermissions.DEFAULT_FILE_PERMISSIONS,
                       LayerConfiguration.DEFAULT_MODIFICATION_TIME));
